...
const Sidebar = ({ isOpen, role }) => {
  ...
  const navs = {
    admin: [
      ...
      { name: 'Manage Code Snippets', icon: Code, path: '/admin/code-snippets' },
      { name: 'Manage Notifications', icon: Bell, path: '/admin/notifications' },
  ...
  return (
    <div
      className="fixed w-[80px] top-0 left-0 h-full bg-[#0e0e23] text-white pt-5 flex"
      style={{
    ...
    >
      ...
      <nav className="flex flex-col gap-1 w-full items-center ">
        {(navs[role] || []).map(({ name, icon: Icon, path }) => {
          ...
          const iconColor = isActive ? '#eb133c' : isHovered ? '#eb133c' : 'white'
      ...
      <div className="fixed bottom-5 w-[100%] flex flex-col justify-center items-center gap-2 ">
         <div className="flex w-[60px] h-[60px] items-center border border-gray-600 justify-center rounded-xl overflow-hidden p-2 hover:bg-gray-800 hover:border-none  cursor-pointer">
         <img src={`${API_BASE_URL}/uploads/profiles/${user?.avatar}`} className='rounded-xl' alt="" />
       </div>
       <div className="flex cursor-pointer w-[60px] h-[60px] bg-[#eb133c] items-center justify-center rounded-xl" onClick={() => {window.localStorage.clear(); window.location.reload() }}>
         <LogOut />
       </div>
      </div>
    </div>
